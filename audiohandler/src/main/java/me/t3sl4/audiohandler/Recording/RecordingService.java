...
public class RecordingService extends Service {
    ...
    public void onCreate() {
        ...
        registerReceiver(stopBroadcastReceiver, filter, Context.RECEIVER_NOT_EXPORTED);
    }
    ...
    public int onStartCommand(Intent intent, int flags, int startId) {
        // Intent null kontrolü
        if (intent == null) {
            Log.w("RecordingService", "Intent is null in onStartCommand");
            return START_STICKY;
        }

        // EXTRA_DATA'nın null kontrolü
        Parcelable parcelableExtra = intent.getParcelableExtra(EXTRA_DATA);
        if (parcelableExtra == null) {
            Log.w("RecordingService", "Intent EXTRA_DATA is null in onStartCommand");
            return START_STICKY;
        }

        // Eğer data null değilse, işlemlere devam edin
    ...
    /*private Notification createNotification() {
        ...
        final PendingIntent disconnectAction = PendingIntent.getBroadcast(this, REQUEST_STOP, disconnect, PendingIntent.FLAG_UPDATE_CURRENT | PendingIntent.FLAG_IMMUTABLE);

        // both activities above have launchMode="singleTask" in the AndroidManifest.xml file, so if the task is already running, it will be resumed
        final PendingIntent pendingIntent = PendingIntent.getActivities(this, REQUEST_OPEN_ACTIVITY, new Intent[]{parentIntent, targetIntent}, PendingIntent.FLAG_UPDATE_CURRENT | PendingIntent.FLAG_IMMUTABLE);
    ...
    private void sendAudioData(byte[] audioData, int length) {
        final Intent broadcast = new Intent(BROADCAST_WAVEFORM);
        broadcast.putExtra(BROADCAST_EXTRA_DATA, audioData);
        broadcast.putExtra("length", length);
        LocalBroadcastManager.getInstance(this).sendBroadcast(broadcast);
    }
